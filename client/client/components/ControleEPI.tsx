...
const ControleEPI: React.FC<ControleEPIProps> = ({ entregas, setEntregas, estoque, setEstoque }) => {
    ...
    // --- LÓGICA DE PAGINAÇÃO (NOVOS ESTADOS) ---
    const [currentPage, setCurrentPage] = useState(1);
    const [itemsPerPage, setItemsPerPage] = useState(100); // Valor padrão de 100 linhas
    ...
    // --- LÓGICA DE PAGINAÇÃO (CÁLCULO DOS ITENS) ---
    const totalPages = Math.ceil(entregas.length / itemsPerPage);
    const startIndex = (currentPage - 1) * itemsPerPage;
    const endIndex = startIndex + itemsPerPage;
    
    // Calcula as entregas a serem exibidas na página atual
    const entregasPaginadas = useMemo(() => {
        return entregas.slice(startIndex, endIndex);
    }, [entregas, startIndex, endIndex]);
    ...
    // Altera o itemsPerPage e volta para a página 1
    ...
    return (
        <div className="space-y-8">
            ...
            {/* Seção de Entregas (AGORA COM PAGINAÇÃO) */}
            <div className="bg-white p-6 rounded-lg shadow-md space-y-4">
                ...
                {/* --- CONTROLES DE PAGINAÇÃO (NOVOS) --- */}
                <div className="flex justify-between items-center flex-wrap gap-2 pt-2">
                    <div className="text-sm text-gray-700">
                        Mostrando **{Math.min(endIndex, entregas.length)}** de **{entregas.length}** registros
                    </div>
                    <div className="flex items-center gap-3">
                        ...
                        >
                            ...
                            <option value={entregas.length}>Todas</option>
                        </select>
                    </div>
                </div>
                ...
                <div className="overflow-x-auto border rounded-lg">
                    <table className="w-full text-sm">
                        ...
                        <tbody>
                            {/* AGORA USANDO entregasPaginadas */}
                ...
                {/* --- CONTROLE DE PÁGINAS (NOVOS) --- */}
                {entregas.length > itemsPerPage && (
