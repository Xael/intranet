...
import { api } from '../utils/api'; // <--- IMPORT ADICIONADO
...
  // --- HANDLERS DE ESTOQUE ---
  // (Esta função já estava correta no arquivo que você me enviou)
...
  // --- FUNÇÃO CORRIGIDA ---
  const handleEditEstoque = async (item: EPIEstoqueItem, field: 'qty' | 'manualOutQty') => { // Adicionado async
...
  // --- FUNÇÃO CORRIGIDA ---
  const handleClearEstoque = async () => { // Adicionado async
    if (window.confirm("ATENÇÃO: Isso limpará TODO o estoque de EPI. Deseja continuar?")) {
      try {
        // Chama a API de restore enviando um array vazio
...
  // --- FUNÇÃO CORRIGIDA ---
  const handleAddEntrega = async (e: FormEvent) => { // Adicionado async
    e.preventDefault();
...
    // Prepara o DTO (Data Transfer Object) para a API, sem o ID
    const newEntryDTO = {
...
    try {
      // Chama a API para salvar e recebe o item salvo (com ID do banco)
      const savedEntry = await api.post('/api/epi', newEntryDTO);
      
      // Atualiza o estado local com o item salvo
...
  
  // --- FUNÇÃO CORRIGIDA ---
  const handleRemoveEntrega = async (id: string) => { // Adicionado async
...
  // --- FUNÇÃO CORRIGIDA ---
  const handleClearEntregas = async () => { // Adicionado async
...
    // (Esta função não precisa de API, está correta)
    if (type === 'estoque') {
...
    reader.onload = async (event) => { // Adicionado async
        try {
...
            // ... (lógica de verificação de formato)
...
            
            // Mapeia para o DTO (sem ID, pois o banco vai criar)
...
                // 2. Atualiza o estado local com a resposta do servidor
...
                // Envia só 'name' e 'qty' pois o backend lida com 'manualOut'
                return { name, qty };
            });
...
                // 2. Atualiza o estado local com a resposta
...
  // --- O RESTANTE DO CÓDIGO (O HTML/JSX) PERMANECE O MESMO ---
  return (
...
i:           <input type="number" value={newEstoque.qty} onChange={e => setNewEstoque({...newEstoque, qty: e.target.value})} placeholder="Quantidade" className="sm:col-span-1 border-gray-300 rounded-md" required />
...
Boolean               <tr key={item.id} className="bg-white border-b hover:bg-gray-50">
...
A             </div>
  m             <button onClick={handleClearEntregas} className="px-3 py-1 text-sm bg-red-600 text-white rounded-md">Limpar</button>
...
Â                     <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
...
            <tbody>
              {entregas.map(item => (
Em               <tr key={item.id} className="bg-white border-b hover:bg-gray-50">
...
line:             <td className="px-6 py-4">{new Date(item.dataEntrega + 'T00:00:00').toLocaleDateString('pt-BR')}</td>
