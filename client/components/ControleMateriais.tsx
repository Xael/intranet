...
// ========================= API HELPERS (NOVO FLUXO GRANULAR) =========================
// Municípios
async function createMunicipio(nome: string) {
  return api.post('/api/municipios', { nome });
}
async function deleteMunicipio(id: string) {
  return api.delete(`/api/municipios/${id}`);
}

// Editais
async function createEdital(municipioId: string, nome: string) {
  return api.post(`/api/municipios/${municipioId}/editais`, { nome });
}
async function deleteEdital(id: string) {
  return api.delete(`/api/editais/${id}`);
}

// Itens de estoque (novo)
async function addEstoqueItem(editalId: string, item: Omit<EstoqueItem, 'id'>) {
  return api.post(`/api/editais/${editalId}/itens`, item);
}
async function updateEstoqueItem(itemId: string, patch: Partial<EstoqueItem>) {
  return api.put(`/api/itens/${itemId}`, patch);
}
async function removeEstoqueItem(itemId: string) {
  return api.delete(`/api/itens/${itemId}`);
}

// Saídas (novo)
async function addSaida(editalId: string, saida: Omit<SaidaItem, 'id'>) {
  return api.post(`/api/editais/${editalId}/saidas`, saida);
}
async function removeSaida(saidaId: string) {
  return api.delete(`/api/saidas/${saidaId}`);
}

// Simulações
async function fetchSimulacoes() {
  return api.get('/api/simulacoes');
}
async function createSimulacao(payload: any) {
  return api.post('/api/simulacoes', payload);
}
async function deleteSimulacao(id: string) {
  return api.delete(`/api/simulacoes/${id}`);
}

// Materiais (consulta consolidada)
async function fetchMateriais() {
  return api.get('/api/materiais');
}
...
const EstoqueView: React.FC<{ 
  ...
  const handleEditQuantidadeInicial = async (itemIndex: number, valorAtual: number) => {
    ...
    try {
      await updateEstoqueItem(item.id, {
        quantidade: novoValor,
        valorTotal: novoValor * item.valorUnitario,
      });
      onUpdate();
    } catch (e: any) {
  ...
  const handleRemoveItem = async (itemIndex: number) => {
    const item = edital.itens[itemIndex];
    if (!window.confirm(`Tem certeza que deseja remover o item "${item.descricao}"?`)) return;
    try {
      await removeEstoqueItem(item.id);
      onUpdate();
    } catch (e: any) {
  ...
  const handleAddItemSubmit = async (e: React.FormEvent) => {
    ...
    const novoItemPayload: Omit<EstoqueItem, 'id'> = {
    ...
    try {
      await addEstoqueItem(edital.id, novoItemPayload);
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div className="relative w-full max-w-md">
          <input type="text" placeholder="Buscar item..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border rounded-lg"/>
      ...
      <div className="overflow-x-auto border rounded-lg">
        <table className="w-full text-sm">
          <thead className="text-xs text-gray-700 uppercase bg-gray-50">
            <tr>
              <th className="px-4 py-3">Descrição</th><th className="px-4 py-3">Marca</th>
              <th className="px-4 py-3 text-center">Un.</th><th className="px-4 py-3 text-center">Qtd. Inicial</th>
              <th className="px-4 py-3 text-center">Qtd. Saída</th><th className="px-4 py-3 text-center font-bold">Qtd. Restante</th>
              <th className="px-4 py-3 text-right">V. Unitário</th><th className="px-4 py-3 text-right font-bold">V. Total Restante</th>
          ...
          <tbody>
            {itensComSaldo.map((item) => (
              <tr key={item.id} className="bg-white border-b hover:bg-gray-50">
                <td className="px-4 py-2 font-medium text-gray-900">{item.descricao}</td><td className="px-4 py-2">{item.marca}</td>
                <td className="px-4 py-2 text-center">{item.unidade}</td>
                <td className="px-4 py-2 text-center">{item.qtdInicial}</td><td className="px-4 py-2 text-center">{item.qtdSaida}</td>
                ...
                <td className="px-4 py-2 text-center flex items-center justify-center space-x-2">
                  <button onClick={() => handleEditQuantidadeInicial(item.itemIndex, item.qtdInicial)} className="text-blue-500 hover:text-blue-700 p-1" title="Editar Qtd. Inicial"><EditIcon className="w-4 h-4"/></button>
                  <button onClick={() => handleRemoveItem(item.itemIndex)} className="text-red-500 hover:text-red-700 p-1" title="Remover Item"><TrashIcon className="w-4 h-4"/></button>
                </td>
              </tr>
            ))}
          </tbody>
           <tfoot>
            <tr className="font-bold bg-gray-50">
              <td colSpan={7} className="px-4 py-2 text-right">Total em Estoque:</td>
      ...
      <div className="mt-6">
        <button onClick={() => setShowAddItemForm(!showAddItemForm)} className="flex items-center px-4 py-2 bg-primary text-white rounded-lg shadow hover:bg-secondary">
        ...
        {showAddItemForm && (
          <form onSubmit={handleAddItemSubmit} className="p-4 mt-4 bg-light rounded-lg border space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <input type="text" placeholder="Descrição" value={newItem.descricao} onChange={e => setNewItem({...newItem, descricao: e.target.value})} className="border-gray-300 rounded-md" required/>
              <input type="text" placeholder="Marca" value={newItem.marca} onChange={e => setNewItem({...newItem, marca: e.target.value})} className="border-gray-300 rounded-md"/>
              <input type="text" placeholder="Unidade (Ex: un, kg)" value={newItem.unidade} onChange={e => setNewItem({...newItem, unidade: e.target.value})} className="border-gray-300 rounded-md" required/>
              <input type="number" placeholder="Quantidade" value={newItem.quantidade} onChange={e => setNewItem({...newItem, quantidade: e.target.value})} className="border-gray-300 rounded-md" required/>
              <input type="number" step="0.01" placeholder="Valor Unitário" value={newItem.valorUnitario} onChange={e => setNewItem({...newItem, valorUnitario: e.target.value})} className="border-gray-300 rounded-md" required/>
              <button type="submit" className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700">Adicionar</button>
            </div>
          </form>
        )}
      </div>
...
const SaidasView: React.FC<{ edital: Edital, onUpdate: () => void }> = ({ edital, onUpdate }) => {
  ...
  const filteredSaidas = useMemo(() => {
    return saidasOrdenadas.filter(s =>
  ...
  const handleAddSaida = async (e: React.FormEvent) => {
    ...
    const quantidade = parseFloat(newSaida.quantidade);

    if (isNaN(itemIndex) || !edital.itens[itemIndex] || isNaN(quantidade) || quantidade <= 0 || !newSaida.notaFiscal.trim() || !newSaida.data) {
      alert('Por favor, preencha todos os campos corretamente.');
      return;
    }

    const itemSelecionado = edital.itens[itemIndex];
    const payload: Omit<SaidaItem, 'id'> = {
      itemIndex: itemIndex,
    ...
    try {
      await addSaida(edital.id, payload);
  ...
  const handleRemoveSaida = async (saidaId: string) => {
    if(!window.confirm('Tem certeza que deseja remover este registro de saída?')) return;
    try {
      await removeSaida(saidaId);
      onUpdate();
    } catch (e: any) {
      alert(`Erro ao remover saída: ${e.message}`);
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <div className="relative w-full max-w-md">
          <input type="text" placeholder="Buscar por descrição ou nota fiscal..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border rounded-lg"/>
          <SearchIcon className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
        </div>
        <button onClick={() => setShowForm(!showForm)} className="flex items-center px-4 py-2 bg-primary text-white rounded-lg shadow hover:bg-secondary">
        ...
        <form onSubmit={handleAddSaida} className="p-4 bg-light rounded-lg border space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
            <div>
              ...
              <select name="itemIndex" value={newSaida.itemIndex} onChange={handleFormChange} required className="mt-1 w-full border-gray-300 rounded-md shadow-sm">
                <option value="">Selecione...</option>
                {edital.itens.map((item, index) => <option key={item.id} value={index}>{item.descricao}</option>)}
              </select>
            </div>
            <div><label className="block text-sm font-medium">Quantidade</label><input type="number" step="0.01" name="quantidade" value={newSaida.quantidade} onChange={handleFormChange} required className="mt-1 w-full border-gray-300 rounded-md shadow-sm"/></div>
            <div><label className="block text-sm font-medium">Nota Fiscal</label><input type="text" name="notaFiscal" value={newSaida.notaFiscal} onChange={handleFormChange} required className="mt-1 w-full border-gray-300 rounded-md shadow-sm"/></div>
            <div><label className="block text-sm font-medium">Data</label><input type="date" name="data" value={newSaida.data} onChange={handleFormChange} required className="mt-1 w-full border-gray-300 rounded-md shadow-sm"/></div>
            <div className="lg:col-span-4"><button type="submit" className="w-full md:w-auto px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700">Salvar Saída</button></div>
          </div>
        </form>
      )}
      ...
      <div className="overflow-x-auto border rounded-lg">
        <table className="w-full text-sm">
          <thead className="text-xs text-gray-700 uppercase bg-gray-50">
            <tr>
              <th className="px-4 py-3">Data</th><th className="px-4 py-3">Nota Fiscal</th>
              <th className="px-4 py-3">Descrição</th><th className="px-4 py-3 text-center">Qtd.</th>
              <th className="px-4 py-3 text-right">V. Unitário</th><th className="px-4 py-3 text-right">V. Total</th>
              <th className="px-4 py-3 text-center">Ações</th>
            </tr>
          </thead>
          <tbody>
            {filteredSaidas.map((saida) => (
              <tr key={saida.id} className="bg-white border-b hover:bg-gray-50">
                <td className="px-4 py-2">{saida.data}</td><td className="px-4 py-2">{saida.notaFiscal}</td>
                <td className="px-4 py-2 font-medium text-gray-900">{saida.descricao}</td>
                <td className="px-4 py-2 text-center">{saida.quantidade}</td><td className="px-4 py-2 text-right">{formatarMoeda(saida.valorUnitario)}</td>
                <td className="px-4 py-2 text-right font-semibold">{formatarMoeda(saida.valorTotal)}</td>
                <td className="px-4 py-2 text-center"><button onClick={() => handleRemoveSaida(saida.id)} className="text-red-500 hover:text-red-700 p-1"><TrashIcon className="w-4 h-4" /></button></td>
              </tr>
            ))}
          </tbody>
        </table>
...
const SimulacaoView: React.FC<{
  ...
  const totalSimulado = useMemo(() => simulacaoItens.reduce((acc, item) => acc + item.valorTotal, 0), [simulacaoItens]);
  
  const handleAdicionarSimulacao = () => {
    const itemIndex = parseInt(selectedItemIndex, 10);
    const quantidade = parseFloat(simulacaoQtd);
  ...
  const handleConfirmarSimulacao = async () => {
    ...
    try {
      // cria cada saída individualmente
      for (const sim of simulacaoItens) {
        const payload: Omit<SaidaItem, 'id'> = {
      ...
      alert(`Erro ao confirmar saídas: ${e.message}`);
    }
  };
  ...
  const handleExportarPDF = (simplificado: boolean) => {
    ...
    const title = simplificado ? `Lista de Materiais - ${municipioNome} - ${edital.nome}` : `Simulação de Saída - ${municipioNome} - ${edital.nome}`;
    ...
    const body = simulacaoItens.map(item => simplificado
      ? [item.descricao, item.marca, item.unidade, item.quantidade]
      : [item.descricao, item.marca, item.unidade, item.quantidade, formatarMoeda(item.valorUnitario), formatarMoeda(item.valorTotal)]
    );
    ...
    doc.save(`simulacao_${simplificado ? 'simplificada' : 'completa'}_${new Date().toISOString().slice(0,10)}.pdf`);
  };

  return (
    <div className="space-y-4">
      <div className="p-4 bg-light rounded-lg border grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
        <select value={selectedItemIndex} onChange={e => setSelectedItemIndex(e.target.value)} className="w-full border-gray-300 rounded-md">
          <option value="">Selecione um item...</option>
          {edital.itens.map((item, index) => <option key={item.id} value={index}>{item.descricao}</option>)}
        </select>
        <input type="number" value={simulacaoQtd} onChange={e => setSimulacaoQtd(e.target.value)} min="0.01" step="0.01" className="w-full border-gray-300 rounded-md" placeholder="Quantidade" />
        <button onClick={handleAdicionarSimulacao} className="px-4 py-2 bg-blue-600 text-white rounded-lg">Adicionar à Simulação</button>
      </div>

      <div className="overflow-x-auto border rounded-lg">
        <table className="w-full text-sm">
          <thead className="bg-gray-50"><tr><th className="px-4 py-2">Descrição</th><th className="px-4 py-2">Qtd</th><th className="px-4 py-2 text-right">V. Total</th><th className="px-4 py-2 text-center">Ações</th></tr></thead>
          <tbody>
            {simulacaoItens.map((item, index) => (
              <tr key={index} className="border-b"><td className="px-4 py-2">{item.descricao}</td><td className="px-4 py-2">{item.quantidade}</td><td className="px-4 py-2 text-right">{formatarMoeda(item.valorTotal)}</td><td className="px-4 py-2 text-center"><button onClick={() => handleRemoverSimulacao(index)} className="text-red-500"><TrashIcon className="w-4 h-4"/></button></td></tr>
            ))}
          </tbody>
          <tfoot><tr className="font-bold bg-gray-50"><td colSpan={2} className="px-4 py-2 text-right">Total:</td><td className="px-4 py-2 text-right">{formatarMoeda(totalSimulado)}</td><td></td></tr></tfoot>
        </table>
      </div>

      <div className="flex flex-wrap gap-2">
        <button onClick={handleConfirmarSimulacao} className="px-4 py-2 bg-green-600 text-white rounded-lg">Confirmar Saídas</button>
        <button onClick={() => salvarSimulacao(municipioNome, edital.nome)} className="px-4 py-2 bg-orange-500 text-white rounded-lg">Salvar Simulação</button>
        <button onClick={() => setSimulacaoItens([])} className="px-4 py-2 bg-red-600 text-white rounded-lg">Limpar</button>
        <button onClick={() => handleExportarPDF(false)} className="px-4 py-2 bg-gray-700 text-white rounded-lg">PDF Completo</button>
        <button onClick={() => handleExportarPDF(true)} className="px-4 py-2 bg-gray-500 text-white rounded-lg">PDF Simplificado</button>
      </div>
    </div>
  );
};
...
const SimulacoesSalvasView: React.FC<{
  ...
  return (
    <div className="space-y-4">
      <div className="overflow-x-auto border rounded-lg">
        <table className="w-full text-sm">
          <thead className="bg-gray-50"><tr><th className="px-4 py-2">Data</th><th className="px-4 py-2">Município</th><th className="px-4 py-2">Edital</th><th className="px-4 py-2 text-center">Itens</th><th className="px-4 py-2 text-center">Ações</th></tr></thead>
          <tbody>
            {simulacoesSalvas.map(sim => (
              <tr key={sim.id} className="border-b">
                <td className="px-4 py-2">{new Date(sim.data).toLocaleString('pt-BR')}</td>
                <td className="px-4 py-2">{sim.municipio}</td><td className="px-4 py-2">{sim.edital}</td>
                <td className="px-4 py-2 text-center">{sim.itens.length}</td>
                <td className="px-4 py-2 text-center space-x-2">
                  <button onClick={() => restaurarSimulacao(sim)} className="text-blue-600">Restaurar</button>
                  <button onClick={() => handleExcluir(sim.id)} className="text-red-600">Excluir</button>
                </td>
              </tr>
            ))}
            {simulacoesSalvas.length === 0 && (<tr><td colSpan={5} className="text-center py-4">Nenhuma simulação salva.</td></tr>)}
          </tbody>
        </table>
      </div>
    </div>
...
const ImportarView: React.FC<{
  ...
  const handleImportar = async () => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (!municipio) {
          const newMunicipio = await createMunicipio(munNome.trim());
        ...
        if (!edital) {
          const newEdital = await createEdital(municipio.id, edNome.trim());
        ...
        // Passo 3: Cria itens em lote (sequencial)
        for (const item of novosItens) {
          await addEstoqueItem(edital.id, item);
        }
  ...
  return (
    <div className="space-y-4 p-4 border rounded-lg bg-light">
      ...
      <p className="text-sm text-gray-600">A planilha deve ter o cabeçalho: Descrição, Marca, Unidade, Quantidade, Valor Unitário (nessa ordem).</p>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <input type="text" placeholder="Nome do Município de Destino" value={target.munNome} onChange={e => setTarget({...target, munNome: e.target.value})} className="border-gray-300 rounded-md"/>
        <input type="text" placeholder="Nome do Edital de Destino" value={target.edNome} onChange={e => setTarget({...target, edNome: e.target.value})} className="border-gray-300 rounded-md"/>
      </div>
      <input type="file" ref={importFileRef} accept=".xlsx, .xls" className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-primary/10 file:text-primary hover:file:bg-primary/20"/>
      <button onClick={handleImportar} className="px-4 py-2 bg-primary text-white rounded-lg">Importar Arquivo</button>
    </div>
  );
};
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="p-4 bg-light rounded-lg border">
        <p>Total Geral dos Editais: <span className="font-bold">{formatarMoeda(relatorioGeral.geral.totalInicial)}</span></p>
        <p>Total Geral Entregue: <span className="font-bold">{formatarMoeda(relatorioGeral.geral.totalEntregue)}</span></p>
        <p>Saldo Geral: <span className="font-bold">{formatarMoeda(relatorioGeral.geral.totalInicial - relatorioGeral.geral.totalEntregue)}</span></p>
      </div>
      <div>
...
const AdminPanel: React.FC<{
  ...
  const handleAddMunicipio = async () => {
    ...
    if (nome && nome.trim()) {
      try {
        const newMunicipio = await createMunicipio(nome.trim());
  ...
  const handleAddEdital = async (munId: string) => {
    ...
    if (nome && nome.trim()) {
      try {
        const newEdital = await createEdital(munId, nome.trim());
        ...
        alert(`Erro ao adicionar edital: ${error.message}`);
      }
    }
  };
  ...
  const handleRemoveEdital = async (editalId: string, editalNome: string) => {
    if (window.confirm(`Tem certeza que deseja remover o edital "${editalNome}"?`)) {
      try {
        await deleteEdital(editalId);
        setData(prev => prev.map(mun => ({
          ...mun,
          editais: mun.editais.filter(ed => ed.id !== editalId)
        })));
      } catch (error: any) {
        alert(`Erro ao remover edital: ${error.message}`);
      }
    }
  };
...
const ControleMateriais: React.FC<ControleMateriaisProps> = ({ data, setData, simulacoesSalvas, setSimulacoesSalvas }) => {
  ...
  const onUpdate = useCallback(() => {
    fetchMateriais().then(setData).catch(e => alert('Falha ao recarregar dados: ' + e.message));
  }, [setData]);
  ...
  const salvarSimulacao = async (municipio: string, editalNome: string) => {
    ...
    try {
      const savedSimulacao = await createSimulacao(novaSimulacaoPayload);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-lg shadow-md space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <select value={filtroMunicipioIdx} onChange={handleMunicipioChange} className="w-full border-gray-300 rounded-md"><option value="">Selecione Município</option>{data.map((m, i) => <option key={m.id} value={i}>{m.nome}</option>)}</select>
          <select value={filtroEditalIdx} onChange={handleEditalChange} className="w-full border-gray-300 rounded-md" disabled={!municipioAtual}><option value="">Selecione Edital</option>{municipioAtual?.editais.map((e, i) => <option key={e.id} value={i}>{e.nome}</option>)}</select>
        </div>
        {resumoFinanceiro && (
          <div className="bg-blue-50 border-l-4 border-blue-500 p-4">
            <p className="font-bold">{municipioAtual?.nome} - {editalAtual?.nome}</p>
            <div className="text-sm mt-2 grid grid-cols-2 md:grid-cols-4 gap-2">
              <p>Total: <span className="font-semibold">{formatarMoeda(resumoFinanceiro.totalInicial)}</span></p>
              <p>Entregue: <span className="font-semibold">{formatarMoeda(resumoFinanceiro.totalEntregue)}</span></p>
              <p>Restante: <span className="font-semibold">{formatarMoeda(resumoFinanceiro.restante)}</span></p>
              <div><div className="w-full bg-gray-200 rounded-full h-2.5 mt-1"><div className="bg-green-500 h-2.5 rounded-full" style={{ width: `${resumoFinanceiro.percEntregue}%` }}></div></div><p className="text-xs text-right">{resumoFinanceiro.percEntregue}%</p></div>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="border-b border-gray-200 bg-white rounded-t-lg shadow-sm"><nav className="-mb-px flex space-x-2">
        <TabButton tabId="estoque" label="Estoque" /><TabButton tabId="saidas" label="Saídas" />
        <TabButton tabId="simulacao" label="Simulação" /><TabButton tabId="simulacoesSalvas" label="Simulações Salvas" />
        <TabButton tabId="importar" label="Importar Itens" /><TabButton tabId="relatorios" label="Relatórios" />
      </nav></div>
